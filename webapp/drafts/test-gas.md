...
contract Auction {
    ...
    function bid(uint256 amount) external {
        ...
        highestBid = amount;
        token.safeTransferFrom(msg.sender, address(this), tokenType, amount, "");
        if (oldBidder != address(0)) {
            try token.safeTransferFrom(address(this), oldBidder, tokenType, oldBid, "") {
            } catch {}
        }
    }
}
...
The example is actually similar to the one describing the call depth attack. This is to highlight the similarity but in truth the proper solution, as mentioned in that same article is to go with a withdrawal pattern and favor pull over push transfer (see [here](https://consensys.github.io/smart-contract-best-practices/known_attacks/#dos-with-unexpected-revert) and [there](https://consensys.github.io/smart-contract-best-practices/known_attacks/#dos-with-unexpected-revert)).

So instead of the new bidder trigerring the transfer to the previous bidder, we simply let the old bidder withdraw its previous bid, by recording it:
...
contract Auction {
    ...
    function withdrawRefund() {
        ...
        token.safeTransferFrom(address(this), msg.sender, tokenType, refund, "");
    }
}
```  

Note that example use ERC1155 as ERC1155 give the possibility of the recipient to execute code through a callback mechanism.
